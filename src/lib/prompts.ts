...
export function lessonWeekPrompt(input: { scope: ScopeSequence; week: number }): string {
  ...
  const genome = input.scope.genomeText?.trim();
  return [
    ...
    genome ? `FUTURE2 EXPERIENCE GENOME (your PRIMARY instructional-design knowledge base — a SEARCHABLE LIBRARY of proven experiential patterns, one per entry, each carrying a student task, competency behaviors, facilitation structure, classroom setup, teacher moves, round progression, reflection, accommodations, extensions, evidence collection, and reusable design principles). Treat it as a pattern library to retrieve from, never a document to summarize or copy:\n${genome}\n` : ``,
    genome ? `GENOME RETRIEVAL for each day: (1) from the LO, EO, Design Model, competency, rubric indicator, dyad, lesson duration, lesson phase, desired energy level, and required student product, search the Genome above; (2) rank entries by competency match, dyad match, Design Model compatibility, lesson-phase compatibility, duration, energy level, student-product similarity, competency behaviors practiced, and quality rating; (3) prefer an exact match, otherwise COMBINE multiple Genome patterns, and only invent a wholly new facilitation when no combination fits. Extract the reusable instructional MECHANISM (for example "students make constrained decisions with incomplete information, revise after new information, and justify tradeoffs") and adapt its scenario, narrative, and materials to today's lesson. NEVER copy a Genome activity verbatim or reuse its name. Actively pull only the assets that strengthen today's lesson: student-task structure, facilitation moves, setup, grouping, materials, escalation rounds, challenge variations, reflection prompts, accommodations, extensions, and evidence-collection methods. Embed the Future2 non-negotiables (Read and Write Every Day, No Opt Out, Urgency, Intentional Groupings) and multiple competency at-bats WITHIN the experience using Genome patterns, never appended as separate activities. The finished lesson should read as though an experienced Future2 writer built it from the strongest Genome ideas while preserving today's Scope & Sequence.` : ``,
