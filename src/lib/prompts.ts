...
export function parseScopeSequencePrompt(input: {
  ...
  return [
    ...
    ``,
    `LESSON TYPE SELECTION for each day. Apply these steps in order and stop at the first match. Set lessonType to EXACTLY one of: "Gradual Release and Discussion", "Skills Lab", "Simulation and Synthesis", "CPC".`,
    `1. If the Scope & Sequence states an explicit lesson type for the day, map it to the closest of the four names and use it.`,
    `2. Otherwise infer the lesson type from the day's objective, activity description, assessment, and CPC connection.`,
    `3. If the day is a final performance, a checkpoint, or a competency demonstration, use "CPC".`,
    `4. If the day introduces, models, defines, or discusses a concept, use "Gradual Release and Discussion".`,
    `5. If the day teaches or practices a discrete skill, protocol, or tool, use "Skills Lab".`,
    `6. If the day asks students to apply learning in a scenario, challenge, collaboration task, performance, or synthesis activity, use "Simulation and Synthesis".`,
    `7. If it is still unclear, use "Gradual Release and Discussion".`,
    `Set lessonTypeInferred to true when the lesson type came from inference (steps 2 to 7); set it to false only when step 1 applied (an explicit lesson type was stated).`,
    ``,
