...
export function canonicalWeekPrompt(input: { scope: ScopeSequence; week: number; genome?: string }): string {
  ...
  return [
    ...
    ``,
    `The Scope & Sequence is AUTHORITATIVE for WHAT each day teaches (objective, rubric indicator, Design Model, sequence, assessment). Do NOT change the progression. You are naming the underlying Access MECHANISM of each day, not reformatting the S&S.`,
    ``,
