...
export function lessonWeekPrompt(input: { scope: ScopeSequence; week: number }): string {
  ...
  return [
    ...
    wkJson,
    ``,
    rubric ? `AUTHORITATIVE RUBRIC (how success is measured):\n${rubric}\n` : ``,
    cpc ? `AUTHORITATIVE CPC (where students finish):\n${cpc}\n` : ``,
    genome ? `FUTURE2 EXPERIENCE GENOME (use to enrich HOW the day is taught — maximize engagement, student autonomy, movement, collaboration, and competency at-bats, without changing WHAT the S&S teaches):\n${genome}\n` : ``,
